fn fft_compute(
  ...
  globalId: vec3<u32>,
  localId: vec3<u32>,
  butterfly: ptr<storage, ButterflyData, read_write>,
  data: ptr<storage, FFTBuffer, read_write>,
) -> void {
  let map_size = numWorkGroups.y * workGroupSize.y;
  let num_stages = findMSB(map_size);
  let id = vec3<u32>(globalId.xy, 0);
  let col = id.x;
  let spectrum = globalId.z;

  if (localId.x >= map_size) return;

  row_shared[row_shared_index(col, 0)] = data[data_in_index(id, spectrum, map_size)];
...
var<workgroup> row_shared: array<f32, 2 * MAX_MAP_SIZE>;
...
fn mul_complex(a: vec2<f32>, b: vec2<f32>) {
